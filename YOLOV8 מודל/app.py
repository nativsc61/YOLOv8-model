...
# זיהוי מנוע עיבוד זמין (CUDA או CPU)
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
print(f"🚀 מריץ מודלים על מכשיר: {DEVICE}")
...
class TrOCRStreamEngine:
    def __init__(self, model_path=DEFAULT_MODEL_PATH, ball_model_path=BALL_MODEL_PATH):
        ...
        if os.path.exists(ball_model_path):
            ...
            print(f"⚠️ אזהרה: קובץ מודל הכדורים לא נמצא. זיהוי כדורים לא יופעל.")
        ...
        # תור משימות OCR מאוחד: (סוג, נתון_מזהה, תמונה)
        self.ocr_queue = Queue(maxsize=30)
    ...
    def _get_youtube_hd_stream_url(self, youtube_url):
        command = [
            'yt-dlp',
            '-f', 'bestvideo[height<=720]+bestaudio/best[height<=720]', # שונה ל-720p כדי להקל על השרת
            '-g',
            youtube_url
        ]
        try:
            ...
            print(f"⚠️ שגיאה בשליפת קישור מיוטיוב: {e}")
            return None
    ...
    def _ocr_worker_loop(self):
        """תור OCR נפרד - מטפל גם ברובוטים, גם בשעון וגם בניקוד מבלי לעצור את YOLO"""
        while True:
            task_type, task_id, crop_bgr = self.ocr_queue.get()
            if not trocr_available or crop_bgr is None or crop_bgr.size == 0:
            ...
            try:
                ...
                if task_type == 'bumper':
                    ...
                    if len(clean_digits) >= 3:
                        ...
                        if matches:
                            self.robot_team_cache[task_id] = str(matches[0])
                ...
                elif task_type in ['score_red', 'score_blue']:
                    digits_only = "".join([c for c in raw_text if c.isdigit()])
                    if digits_only:
                        val = int(digits_only)
                        if task_type == 'score_red':
    ...
    def _yolo_loop(self):
        """לולאת זיהוי ומעקב רציפה ללא מניעות"""
        while True:
            ...
            # הרצת YOLO ראשי ב-imgsz=640 ועל המכשיר המוגדר
            results = self.yolo_model.track(
                frame_to_process, persist=True, tracker="bytetrack.yaml",
                conf=0.15, imgsz=640, device=DEVICE, verbose=False
            )

            can_run_ocr = (time.time() - self.last_ocr_time) >= 1.5
            ...
            if results and len(results) > 0 and results[0].boxes is not None:
                ...
                for box in boxes:
                    ...
                    # העברת זיהוי השעון והניקוד לתור האסינכרוני
                    if cls_name in ['TIME', 'CLOCK'] and can_run_ocr:
                        ...
                        if time_crop.size > 0 and self.ocr_queue.qsize() < 10:
                            self.ocr_queue.put_nowait(('time', None, time_crop))

                    elif cls_name in ['SR', 'SB', 'SCORE_RED', 'SCORE_BLUE'] and can_run_ocr:
                        x1_p, y1_p = max(0, int(x1)), max(0, int(y1))
                        x2_p, y2_p = min(width, int(x2)), min(height, int(y2))
                        score_crop = frame_to_process[y1_p:y2_p, x1_p:x2_p]
                        task = 'score_red' if cls_name in ['SR', 'SCORE_RED'] else 'score_blue'
                        if score_crop.size > 0 and self.ocr_queue.qsize() < 10:
                            self.ocr_queue.put_nowait((task, None, score_crop))

                    else:
                        if track_id is not None and track_id not in self.robot_team_cache:
                            x1_p, y1_p = max(0, int(x1) - 5), max(0, int(y1) - 5)
                            x2_p, y2_p = min(width, int(x2) + 5), min(height, int(y2) + 5)
                            bumper_crop = frame_to_process[y1_p:y2_p, x1_p:x2_p]
                            
                            if bumper_crop.size > 0 and self.ocr_queue.qsize() < 10:
                                try:
                                    self.ocr_queue.put_nowait(('bumper', track_id, bumper_crop))
                                except Exception:
                                    pass
            ...
            # דילוג פריימים למודל הכדורים (רץ פעם ב-3 פריימים כדי לחסוך מעבד)
            if self.ball_model is not None and self.frame_counter % 3 == 0:
                ball_results = self.ball_model(frame_to_process, conf=0.15, imgsz=640, device=DEVICE, verbose=False)
